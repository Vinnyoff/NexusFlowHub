    ...
    <AppLayout>
      <div className="space-y-6 print:hidden">
        <div>
          ...
          <p className="text-muted-foreground">Gere identificação física utilizando o Código Interno (EAN-8).</p>
        </div>
          ...
          <Card className="lg:col-span-2 border-none shadow-sm">
            <CardHeader className="pb-3">
              <div className="relative">
                ...
                <Input 
                  placeholder="Buscar por nome, marca ou códigos..." 
                    ...
                    {filteredProducts.map((product) => (
                      <TableRow key={product.id} className={selectedProduct?.id === product.id ? "bg-primary/5" : ""}>
                        ...
                        <TableCell className="text-right pr-6">
                          ...
                          >
                            {selectedProduct?.id === product.id ? <CheckCircle2 className="h-4 w-4" /> : "Selecionar"}
                          </Button>
                        </TableCell>
                      </TableRow>
                    ))}
            ...
            <Card className="border-none shadow-sm">
              ...
              <CardContent className="space-y-4">
                <div className="space-y-2">
                  <label className="text-[10px] font-bold uppercase text-muted-foreground tracking-wider">Quantidade</label>
                  <Input 
                    type="number" 
                    min="1" 
                    value={quantity}
                ...
                <div className="p-4 border border-dashed rounded-xl bg-muted/20 flex flex-col items-center justify-center min-h-[180px]">
                  {selectedProduct ? (
                    ...
                    <div className="text-center text-muted-foreground">
                      ...
                      <p className="text-xs italic">Selecione um produto para prévia.</p>
                    </div>
                  )}
                </div>
            ...
            <Card className="bg-muted/50 border-none">
              <CardContent className="p-4 flex gap-3">
                ...
                <p className="text-[10px] text-muted-foreground leading-relaxed">
                  As etiquetas usam o padrão <strong>EAN-8</strong> interno. O PDV aceitará tanto este quanto o código de barras original do fabricante.
                </p>
              </CardContent>
            </Card>
          </div>
          ...
          {Array.from({ length: quantity }).map((_, i) => (
            ...
            >
              ...
              <div className="text-center w-full">
                <p className="text-[9px] text-black font-bold line-clamp-1 leading-tight">{selectedProduct?.name}</p>
                <p className="text-[7px] text-black font-medium line-clamp-1 opacity-80">{selectedProduct?.model || selectedProduct?.size || ''}</p>
              </div>
              <div className="flex flex-col items-center flex-1 justify-center py-1 scale-95">
                <Barcode 
                  value={selectedProduct?.internalCode || "00000000"} 
                  format="EAN8"
                  width={1.5} 
                  height={35} 
                  fontSize={8}
              ...
              <p className="text-[13px] font-black text-black">R$ {selectedProduct?.price?.toFixed(2)}</p>
            </div>
          ))}
        </div>
      </div>
